...
func _build_contracts_for_current_system() -> void:
	...
	# Safety check – must have a valid system
	if sys_id == "":
		contracts = []
		info_label.text = "No system selected."
		return

	# Safety check – try location-first, but location may be blank on first load
	var generated: Array = []
	if loc_id != "":
		generated = Contracts.generate_contracts_for_location(loc_id, 4)

	# If location contracts failed or returned empty, fall back to system-level
	if generated.is_empty():
		generated = Contracts.generate_contracts_for_system(sys_id, 4)
	...
	if contracts.is_empty():
		...
		# If we generated by location, label should reflect that
		if loc_id != "":
